                    ...
                    <div class="row">
                        <div class="col-sm-12 lobipanel-parent-sortable ui-sortable" data-lobipanel-child-inner-id="cpa90TSeTV">
                            <div class="panel panel-bd lobidrag lobipanel lobipanel-sortable" data-inner-id="cpa90TSeTV" data-index="0">
                                <div class="panel-heading ui-sortable-handle">
                                    <div class="btn-group"> 
                                       <a class="btn btn-success" href="feedback/create"> <i class="fa fa-plus"></i> Send Feedback</a> 
                                    </div>
                        ...
                        @if($data)
							@foreach($feedback as $feed)
									@if(Auth::user()->id == $feed->receiverId || Auth::user()->id == $feed->senderId)
										<tr style="text-align: center;">
											...
											@if(Auth::user()->type == "admin")
												<td>    
			                                       {!! Form::open(['action'=> ['feedbacksController@update',$feed->id], 'method'=>'POST'], ['class'=>'col-sm-6']) !!}
													
													 <input id="status" name="status" type="hidden" value="{{$feed->status}}" hidden>

			                                        {{Form::hidden('_method', 'PUT')}}
		@if ($feed->status == "Unsolved")
             {{ Form::submit('Solved', ['class'=>'btn btn-success','style'=>'background-color: #26a69a, border-color:#26a69a'])}}
        @else
            {{ Form::submit('Unsolved', ['class'=>'btn btn-warning form-button','style'=>'background-color: #26a69a, border-color:#26a69a'])}}
        @endif
